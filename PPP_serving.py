...
from PPP_config import get_pertubation_list, get_config
    ...
    for sample in samples_:
        ...
        for name, pertubation in PERTUBATION_LIST:
            ...
            for folder in os.listdir(MODEL_PATH):
                ...
                mu = round(mu[0], 2)
                sigma = round(sigma[0], 3)
                real_score = classifier.score(X_test_perturbed, y_test)

                if PLOT:
                    list_real_score.append((int(index), real_score))
                    list_gp_score.append((int(index), mu))
            ...
            if PLOT:
                list_real_score.sort(key=lambda tup: tup[0])
                list_gp_score.sort(key=lambda tup: tup[0])
                ppp_plot.plot_real_gp(list_real_score, list_gp_score)
                if PLOT_ALL_IN_ONE:
